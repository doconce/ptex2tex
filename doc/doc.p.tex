...
provided one applies the \code{-shell-escape} option to \code{latex}:
!bsys
Unix/DOS> latex -shell-espace myfile.tex
!esys
The reason for requiring this option is that the resulting \code{.tex}
file includes \code{minted.sty}, which assumes the the Pygments program
is run, and this program cannot be run without the \code{-shell-escape}
option.
In
