...
the way you want it to. Let's look at an example involving a black box:
\bpy
...
\epy
\edat
With Ptex2tex the box expands as a certain environment, e.g. \code{mybox}:
\bdat
\bmybox
import sys; print 'script name is', sys.argv[0]
\emybox
\edat
Now, we can define what type of box we want in a configuration file. The
...
a box. Suppose you want the box to have a complete different look:
\bmata
import sys; print 'script name is', sys.argv[0]
\emata
which in pure \LaTeX~looks like:
\begin{SaveVerbatim}{tmp2}
\vspace{4pt}
\begin{Verbatim}[numbers=none,frame=lines,label=\fbox{Code},
fontsize=\footnotesize,labelposition=topline,framesep=2.5mm,
framerule=1pt]
import sys; print 'script name is', sys.argv[0]
\end{Verbatim}
\end{SaveVerbatim}
\begin{shadedquotedat}
\fontsize{9pt}{9pt}
\UseVerbatim{tmp2}
\end{shadedquotedat}
\noindent
...
make any changes to your document itself at all!
...
\line(1,0){374}\\
 \\
This makes Ptex2tex a very convenient tool when writing large \LaTeX~documents about software.
...
and the terminal session is typeset using the environment key 'sys' ('System').
...
\subsection{Configuration file.}\label{sec:conffile}
...
\LaTeX~commands, resulting in plain \code{.tex} files. For instance will the
...
bstretch
\edat
Here is an example where all these options are used:
\bdsni
[Program]
define   = True
newenv   = \providecommand{\shadedquote}{}
           \renewenvironment{shadedquote}[1][]{
           \definecolor{shadecolor}{rgb}{0.87843, 0.95686, 1.0}
           \bgroup\rmfamily
           \fboxsep=0mm\relax
           \begin{shaded}
           \list{}{\parsep=-2mm\parskip=2mm\topsep=0pt\leftmargin=2mm
           \rightmargin=2\leftmargin\leftmargin=4pt\relax}
           \item\relax}
           {\endlist\end{shaded}\egroup}
breplace = \begin{shadedquote}
           \fontsize{9pt}{9pt}
           \begin{Verbatim}
...
The example above merely shows what the code looks like, the details are given
next.
...
the environment 'Program', we write:
\bdat
[names]
pro = Program
\edat
Hence we can use \code{\}\code{bpro} to indicate the
beginning of the 'Program' environment, and \code{\}\code{epro} to indicate the end.
...
this environment. Currently the
...
\bdsni
breplace = \begin{Verbatim}[fontsize=\%(fontsize)s,tabsize=8,
           baselinestretch=%(bstretch)s,
	   defineactive=\def\#{\itshape \CommentChar},fontfamily=tt]
ereplace = \end{Verbatim}
#          This is a config file comment
	   \noindent %%This is a \LaTeX comment
bstretch = 0.85
fontsize = footnotesize
\edsni
...
\code{ptex2tex.sty}. 
If installed correctly, so that \LaTeX~can find it, one can simply use
\bdat
\usepackage{ptex2tex}
\edat
instead of specifying all necessary packages. Alternatively, one can simply
...
All the environments included with Ptex2tex are a result of the needs of the
author. One might want to change these, or add new ones. Several of the
environments are plain copies of each other, at the time of writing this. 
Here is a short overview of the different environments:
\bpro
The environment 'Program' is used for this text.
It is meant for longer program code.
\epro
\bsni
The environment 'Snippet' is used for this text.
It is meant for shorter program code.
\esni
\bcod
The environment 'Programsingle' is used for this text.
It is meant for one-line program code.
\ecod
\bdat
The environment 'Data' is used for this text.
It is meant for the inclusion of plain data files.
\edat
\bdsni
The environment 'Datasnippet' is used for this text.
It is meant for the inclusion of short pieces of plain data files.
\edsni
\bsys
The environment 'System' is used for this text.
It is meant for writing longer commands in a terminal window.
\esys
\bslin
The environment 'Systemline' is used for this text.
It is meant for writing a single line in a terminal window.
\eslin
\bcc
The environment 'Code' is used for this text.
This is plain, simple verbatim.
\ecc
\bccl
The environment 'CodeLine' is used for this text.
This is plain, simple verbatim with numbering of lines.
\eccl
\code{The keyword '\}\code{code' is used for this text.}
\bipy The environment 'IPython' is used for this text.
It is meant for code statements in the interactive IPython shell.
Notice that the text should start immediately after the keyword
defining the beginning of the environment in order for the text 
to start after the IPython prompt.
\eipy
\bpy 
The environment 'Python' is used for this text.
It is meant for longer code statements in the regular Python shell.
\epy
\bplin
The environment 'Pythonline' is used for this text.
It is meant for single line code statements in the regular Python shell.
\eplin
\bver
The environment 'Verb' is used for this text.
It is a simple verbatim environment.
\ever
\bwarn
The environment 'Warning' is used for this text.
warning.eps needs to be accessible by \LaTeX.
\ewarn
\brule
The environment 'Tip' is used for this text.
tip.eps needs to be accessible by \LaTeX.
\erule
\bsumm
The environment 'Note' is used for this text.
note.eps needs to be accessible by \LaTeX.
\esumm
\bmata
The environment 'MatlabA' is used for this text.
It is another example of how to display code. For now, there is no way of
setting the heading ('Code') for each time the environment is used. This is on
the TODO list.
\emata
