...
namely \code{\code}. This keyword can be used anywhere in the text, as
long as it is contained within a single line. This allows the use of any
character, we can for instance write
\bcod
\code{}@#_%}
\ecod
without \LaTeX~giving
us any errors. The result is similar to the \code{\texttt}
...
The \code{code} command is translated to a \code{Verb} command
(requiring the \code{fancyvrb} package). For example
\bcod
\code{my_func}
\ecod
becomes
